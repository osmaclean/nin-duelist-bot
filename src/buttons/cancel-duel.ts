...
export async function handleCancelDuel(interaction: ButtonInteraction) {
  ...
  if (!duel) {
    await interaction.followUp({ content: 'Duelo não encontrado.', ephemeral: true });
    return;
  }

  const allowedStatuses = ['PROPOSED', 'ACCEPTED', 'IN_PROGRESS'];
  if (!allowedStatuses.includes(duel.status)) {
    await interaction.followUp({ content: 'Este duelo não pode mais ser cancelado.', ephemeral: true });
    return;
  }
  ...
  if (!participantIds.includes(interaction.user.id)) {
    await interaction.followUp({ content: 'Apenas os participantes do duelo podem cancelá-lo.', ephemeral: true });
    return;
  }

  const updated = await cancelDuel(duelId);
  if (!updated) {
    await interaction.followUp({ content: 'Erro ao cancelar duelo.', ephemeral: true });
    return;
  }
